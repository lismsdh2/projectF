...
//			e.printStackTrace();
	...
	public void insertTask(TaskDto taskDto, int classno) {
		...
		try {
			...
			pstmt.setBlob(5, (Blob) taskDto.getAttachedFile());
			pstmt.setString(6, taskDto.getTcFile());
			pstmt.setInt(7, classno);
			pstmt.setInt(8, taskDto.getPerfectScore());
	...
	// 수정필요
	public void updateReport(TaskDto report) {
		//DB연결
		connectionJDBC();
		
		try {
			String sql = "update task set task_name=?, task_desc=?, expire_date=?, attachedFile=?, perfect_Score=?	where task_no=?;";

			pstmt = connection.prepareStatement(sql);

			pstmt.setString(1, report.getTcTitle());
			pstmt.setString(2, report.getTcDesc());
			pstmt.setString(3, report.getTcExpireDate().toString());
			pstmt.setString(4, report.getTcFile());
			pstmt.setInt(5, report.getPerfectScore());
			pstmt.setInt(6, report.getTcNo());
...
//				report.setTcExpireDate(LocalDate.parse(rs.getString("expireDate"), DateTimeFormatter.ISO_DATE));
				...
				task.setTcFile(rs.getString("attachedFile"));
				task.setPerfectScore(rs.getInt("perfect_score"));
...
//				report.setTcExpireDate(LocalDate.parse(re.getString("expireDate"), DateTimeFormatter.ISO_DATE));
				...
				report.setTcFile(re.getString("attachedFile"));
